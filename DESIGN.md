...
Corners are moderate and consistent: `rounded-lg` (8px) for buttons and inputs, `rounded-xl` (12px) for every card and frame (`Card`, `BrowserFrame`, sign-in card). There is no ornamental hardware silhouette in this world — no corner brackets, no drawn seal or stamp — a deliberate departure from both prior worlds' authored-SVG ornament devices. The one recurring drawn mark is `RowDot`, a plain filled circle with an inset ring and a checkmark glyph on activation, functioning as the SaaS equivalent of a checkbox rather than a diagram element.
